...
class POMDP:
    ...
    def convert_string_to_belief(self, line):
        tokens = line.split(' ')
        for i, t in enumerate(tokens):
            self.belief[i] = double(t)

    def get_initial_belief(self, file_name):
        file = open(file_name, "r")
        flag = False
        for line in file:
            if flag == True:
                self.belief = self.convert_string_to_belief(line)
            if line == "start:":
                flag = True
            print line,
        return 0
    ...
    def __init__(self, pomdp_env_filename, pomdp_policy_filename, prior):
        ...
        # If prior not specified, read the belief state from pomdp file
        if prior is None:
            self.belief = self.pomdpenv.belief
        else:
            self.belief = prior
...
class POMDPEnvironment:
    ...
    def __get_start(self, i):
        ...
        for index, token in enumerate(line.split(' ')):
            self.belief.append(double(token))
        return i + 1
    ...
    def __get_states(self, i):
        ...
        line = self.contents[i]
        total_states = int(line.split()[1:][0])
        for j in xrange(0, total_states):
            self.states.append(str(j))
        return i + 1

    def __get_actions(self, i):
        line = self.contents[i]
        total_actions = int(line.split()[1:][0])
        for j in xrange(0, total_actions):
            self.actions.append(str(j))
        return i + 1

    def __get_observations(self, i):
        line = self.contents[i]
        total_observations = int(line.split()[1:][0])
        for j in xrange(0, total_observations):
            self.observations.append(str(j))
        return i + 1
    ...
    def update_belief(self, prev_belief, action_num, observation_num):
        ...
        for s_prime in range(len(self.states)):
            if (action_num, s_prime, observation_num) not in self.Z:
                p_o_prime = 0
            else:
                p_o_prime = self.Z[(action_num, s_prime, observation_num)]
            summation = 0.0
            for s in range(len(self.states)):
                if (action_num, s, s_prime) not in self.T:
                    p_s_prime = 0
                else:
